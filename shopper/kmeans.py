...
def main():
    ...
    parser.add_argument('--pos_sp', type=int, default=2)
    ...
    tr = train_dataset[:]['src'][:, 1:, idx1:idx2].reshape(-1, 2)
    pr = train_dataset[:]['trg'][:, :, idx1:idx2].reshape(-1, 2)
    t = torch.cat((tr, pr), 0)
    t= t.cpu().numpy()
    ...
    te = test_dataset[:]['src'][:, 1:, idx1:idx2].reshape(-1, 2)
    test=te

    if args.dataset_name!="trajnet":
        pe = test_dataset[:]['trg'][:, :, idx1:idx2].reshape(-1, 2)
    ...
    print('Saving...')
    scipy.io.savemat(f'./clusters/clusters_'+model_name+'_'+str(args.num_clusters)+'.mat',{'centroids':center})
    '''
    print('1')
    ...
    print('6')
    scipy.io.savemat(f'./clusters'+model_name+'_'+str(args.num_clusters)+'.mat',{'centroids':center,"counts":cluster_count_tr})
    '''
