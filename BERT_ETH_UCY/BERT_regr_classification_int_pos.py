...
def main():
    ...
    outdir=f'./results/Timestamp_Prediction'
    try:
        os.mkdir(outdir)
    except:
        pass
    ...
    outdir=f'./results/Timestamp_Prediction/Regressive'
    try:
        os.mkdir(outdir)
    except:
        pass
    ...
    outdir=f'./results/Timestamp_Prediction/Regressive/'+'BERT_'+dict_folder_data[args.data_type]+'_'+dict_folder_goal[args.goal_type]
    try:
        os.mkdir(outdir)
    except:
        pass
    ...
    generator=GeneratorTS(768,1).to(device)

    
    if args.goal_type == 2:
        optim = NoamOpt(768, args.factor, len(tr_dl)*args.warmup, torch.optim.Adam(list(goal_model.parameters())+list(a.parameters())+list(model.parameters())+list(generator.parameters()), lr=0, betas=(0.9, 0.98), eps=1e-9))
    else:
        optim = NoamOpt(768, args.factor, len(tr_dl)*args.warmup, torch.optim.Adam(list(a.parameters())+list(model.parameters())+list(generator.parameters()), lr=0, betas=(0.9, 0.98), eps=1e-9))
    ...
    if args.goal_type != 0:
        true_int_pos_tr = randint(0, args.preds, size=(args.max_epoch*len(tr_dl), args.batch_size))
        true_int_pos_val = randint(0, args.preds, size=(args.max_epoch*len(val_dl), args.batch_size))
        true_int_pos_test = randint(0, args.preds, size=(args.max_epoch*len(test_dl), args.batch_size))

        selected_int_pos_tr = randint(args.obs, args.obs+args.preds, size=(args.max_epoch*len(tr_dl), args.batch_size))
        selected_int_pos_val = randint(args.obs, args.obs+args.preds, size=(args.max_epoch*len(val_dl), args.batch_size))
        selected_int_pos_test = randint(args.obs, args.obs+args.preds, size=(args.max_epoch*len(test_dl), args.batch_size))
    else:
        true_int_pos_tr = randint(0, args.preds+1, size=(args.max_epoch*len(tr_dl), args.batch_size))
        true_int_pos_val = randint(0, args.preds+1, size=(args.max_epoch*len(val_dl), args.batch_size))
        true_int_pos_test = randint(0, args.preds+1, size=(args.max_epoch*len(test_dl), args.batch_size))

        selected_int_pos_tr = randint(args.obs, args.obs+args.preds+1, size=(args.max_epoch*len(tr_dl), args.batch_size))
        selected_int_pos_val = randint(args.obs, args.obs+args.preds+1, size=(args.max_epoch*len(val_dl), args.batch_size))
        selected_int_pos_test = randint(args.obs, args.obs+args.preds+1, size=(args.max_epoch*len(test_dl), args.batch_size))
    ...
    while epoch<args.max_epoch:
        ...
        for id_b, batch in enumerate(tr_dl):
            ...
            target = ((batch['trg'][:,:,idx1:idx2]-mean)/std).to(device)
            ...
            true_posit = target[np.arange(target.shape[0]), true_int_pos_tr[epoch*len(tr_dl)+id_b, :n_in_batch], :]
            net_input[np.arange(net_input.shape[0]), selected_int_pos_tr[epoch*len(tr_dl)+id_b, :n_in_batch], :2] = true_posit
            net_input[np.arange(net_input.shape[0]), selected_int_pos_tr[epoch*len(tr_dl)+id_b, :n_in_batch], 2] = 2             # CHECKPOINT MASK
            true_label = torch.tensor(args.obs+true_int_pos_tr[epoch*len(tr_dl)+id_b, :n_in_batch]).to(device)
            ...
            pred=generator(out[0]).squeeze(-1)


            if id_b==0 and args.verbose:
                print("\nNEW  timestamp for the checkpoint:", selected_int_pos_tr[id_b,0])
                print("TRUE timestamp for the checkpoint:", true_label[0].item())
                print("PRED TIMESTAMP", F.softmax(pred, dim=1).argmax(dim=1).cpu().numpy()[0])

                print("\nINPUT")
                print(net_input[0, :])
                print("TRUE RELAT POSITION")
                print(torch.cat((inp, target),1)[0, :])

          
            loss_traj = F.cross_entropy(pred, true_label, reduction='mean')

            abs_dist += np.sum(np.abs(F.softmax(pred, dim=1).argmax(dim=1).cpu().numpy() - true_label.cpu().numpy()))
        ...
        with torch.no_grad():
            ...
            for id_b, batch in enumerate(val_dl):
                ...
                target = ((batch['trg'][:,:,idx1:idx2]-mean_val)/std_val).to(device)
                ...
                true_posit = target[np.arange(target.shape[0]), true_int_pos_val[epoch*len(val_dl)+id_b, :n_in_batch], :]
                net_input[np.arange(net_input.shape[0]), selected_int_pos_val[epoch*len(val_dl)+id_b, :n_in_batch], :2] = true_posit
                net_input[np.arange(net_input.shape[0]), selected_int_pos_val[epoch*len(val_dl)+id_b, :n_in_batch], 2] = 2             # CHECKPOINT MASK
                true_label = torch.tensor(args.obs+true_int_pos_val[epoch*len(val_dl)+id_b, :n_in_batch]).to(device)
            ...
            for id_b, batch in enumerate(test_dl):
                ...
                true_posit = target[np.arange(target.shape[0]), true_int_pos_test[epoch*len(test_dl)+id_b, :n_in_batch], :]
                net_input[np.arange(net_input.shape[0]), selected_int_pos_test[epoch*len(test_dl)+id_b, :n_in_batch], :2] = true_posit
                net_input[np.arange(net_input.shape[0]), selected_int_pos_test[epoch*len(test_dl)+id_b, :n_in_batch], 2] = 2            # CHECKPOINT MASK
                true_label = torch.tensor(args.obs+true_int_pos_test[epoch*len(test_dl)+id_b, :n_in_batch]).to(device)
    ...
    save_folder = './results/Timestamp_Prediction/Regressive/'+'BERT_'+dict_folder_data[args.data_type]+'_'+dict_folder_goal[args.goal_type]+'/'
    file_name =  'regr_correct_checkpoint'+'_'+args.dataset_name+'_data'+str(args.data_type)+'_goal'+str(args.goal_type)+'_Epoch'+str(args.max_epoch)+'.csv'
